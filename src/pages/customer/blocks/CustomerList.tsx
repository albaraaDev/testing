...
import { getCustomers, CustomerDetails, exportCustomerReport } from '@/api/customers.ts';
import { Paginated } from '@/api/common.ts';
import { Download, Filter, Loader2 } from 'lucide-react';
import clsx from 'clsx';
// import CustomersGridView from '../components/CustomersGridView.tsx';
// import CustomersCardsView from '../components/CustomersCardsView.tsx';
...
import CustomersCardsView from '../components/CustomersCardsView';
...
function CustomerList({ fetchCustomerStats }: CustomerListProps) {
  const intl = useIntl();
  ...
  const handleExport = async () => {
    try {
      startExporting('customerList');
      const response = await exportCustomerReport({
        pageIndex: 0,
        pageSize: 100
      });
    ...
    <div className="min-h-0 card">
      <div className="px-4 sm:px-7 pt-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="card-title">
          <h3>
            <FormattedMessage id="CUSTOMER.LIST.CUSTOMER" />
          </h3>
          <h4 className="text-sm font-thin text-[#B5B5C3]">
            <FormattedMessage
              id="CUSTOMER.LIST.COUNT"
              values={{ count: customers?.totalCount ?? 0 }}
            />
          </h4>
        </div>
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-4">
            <button className="flex items-center gap-2 px-3 py-2 text-gray-600 rounded-lg border hover:bg-gray-50">
              <Filter size={16} />
              <span>
                <FormattedMessage id="CUSTOMER.ACTIONS.FILTERS" />
              </span>
              <span className="flex items-center justify-center w-5 h-5 text-xs bg-gray-100 rounded-full">
                <FormattedMessage id="CUSTOMER.ACTIONS.FILTERS.COUNT" values={{ count: 2 }} />
              </span>
            </button>
          </div>
          ...
          <div className="relative">
            ...
            <DebouncedSearchInput
              ...
              placeholder={intl.formatMessage({ id: 'CUSTOMER.ACTIONS.SEARCH.PLACEHOLDER' })}
      ...
      <div className="gap-cols responsive-card">
        <div className="card-body pt-2 px-2 sm:px-6 pb-7">
          {viewMode === 'grid' ? (
            <CustomersGridView searchQuery={searchQuery} refetchStats={fetchCustomerStats} />
          ) : (
            <CustomersCardsView searchQuery={searchQuery} refetchStats={fetchCustomerStats} />
          )}
        </div>
      </div>
    </div>
